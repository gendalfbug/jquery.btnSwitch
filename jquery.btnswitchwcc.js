/**
 * jQuery Button Switch Plugin + WCC (with ConfirmCall)
 * Version 1.0.0
 * 
 * Minimal Usage: $('#switch').btnSwitchWCC();
 * Settings:
...
(function($) {
    $.fn.btnSwitchWCC = function(options) {
        ...
        var confirmCallEvent = function (callbackSwitch) {
                settings.ConfirmCall(callbackSwitch.bind());
        };
        ...
        return this.each(function() {
            ...
            switch(settings.Theme) {
                ...
                default:
                    ...
                    btnSwitchWCC.html(settings.ToggleState == settings.OnValue ? switchOnTpl : switchOffTpl);

                    $('#bsh-' + id + ' button').on('click', function() {
                        var sel = $(this).data('title');

                        //todo: settings.ConfirmCall
                ...
                case 'Light':
                    ...
                    $('#sw-light-' + dataToggle).on('click', function() {
                        ...
                        if (settings.ConfirmChanges && $.isFunction(settings.ConfirmCall)){
                            var _this = this;
                            confirmCallEvent(function (resultConfirm) {
                                if(resultConfirm){
                                    if (settings.HiddenInputId != false) {
                                        $('#' + settings.HiddenInputId).prop('value', selValue);
                                    }

                                    lightClickEvent($(_this), id, state, selValue);
                                }
                            })
                        }else if (settings.ConfirmChanges) {
                            if (confirm(settings.ConfirmText)) {
                                if (settings.HiddenInputId != false) {
                                    $('#' + settings.HiddenInputId).prop('value', selValue);
                                }
                        ...
                        if (settings.ConfirmChanges && $.isFunction(settings.ConfirmCall)){
                            ...
                            confirmCallEvent(function (resultConfirm) {
                                if(resultConfirm){
                                    ...
                                    iosClickEvent($(_this), id, state, selValue);
                                }
                            })
                        }else if (settings.ConfirmChanges) {
                            if (confirm(settings.ConfirmText)) {
                        ...
                        if (settings.ConfirmChanges && $.isFunction(settings.ConfirmCall)){
                            ...
                            confirmCallEvent(function (resultConfirm) {
                                if(resultConfirm){
                                    ...
                                    androidClickEvent($(_this), id, state, selValue);
                                }
